...
def deploy():
    matches = crm_containers()
    if len(matches) != 1:
        raise RuntimeError("Expected exactly one CRM container; refusing an ambiguous update")
    container = matches[0]
    if container["Config"]["Image"] != IMAGE:
        raise RuntimeError("CRM image does not match the fixed deployment target")
    run("docker", "pull", IMAGE)
    try:
        run("docker", "stop", "--time", "60", container["Id"])
        state = json.loads(run("docker", "inspect", "--format", "{{json .State}}", container["Id"], capture=True))
        if state["Running"] or state["ExitCode"] == 137 or state.get("OOMKilled"):
            raise RuntimeError("CRM did not stop gracefully; refusing to replace it")
        run("once", "update", HOST, "--auto-update=false")
    except Exception:
        ...
        try:
            ...
            print("CRM recovery failed; operator intervention required", file=sys.stderr)
        raise
...
def main():
    ...
    with open("/run/lock/deploy-dealcontext.lock", "a") as lock:
