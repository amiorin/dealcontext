#!/usr/bin/env python3
"""Command-line client for a DealContext CRM server. Python 3 standard library only.
...
ENV = ['DEALCONTEXT_URL', 'DEALCONTEXT_AGENT_EMAIL', 'DEALCONTEXT_AGENT_PASSWORD']
...
def cache_file(cfg):
    ...
    return Path(base) / 'dealcontext' / (key + '.json')
...
def send(cfg, method, path, body=None, token=None):
    ...
    try:
        ...
        if 300 <= status < 400:
            raise Fail(1, f'HTTP {status}: the server redirects to {error.headers.get("Location")}. Set DEALCONTEXT_URL to the final address.')
...
def check(cfg):
    ...
    for line in differences:
        say(line, sys.stdout)
    say('The server is authoritative: run `dc.py schema` and follow the server\'s error messages where the reference files disagree. '
...
def parse(argv):
    ...
    parser = argparse.ArgumentParser(prog='dc.py', parents=[pretty], description='DealContext CRM client. Reads with SQL, writes through the records API. There is no delete command: agents cannot delete records.',
    ...
    add('get', 'read one record', ('collection', 'collection name'), ('id', 'record id'))
...
def main():
    try:
        ...
        say(f'dc.py: unexpected {type(error).__name__}: {error}')
        return 1
