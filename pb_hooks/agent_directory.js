...
function sync(e, deleting) {
  ...
  app.runInTransaction((txApp) => {
    ...
    try {
      e.next();
      const rows = txApp.findRecordsByFilter("agent_directory", "id = {:id}", "", 1, 0, {id: e.record.id});
      if (deleting) {
        if (rows.length) txApp.delete(rows[0]);
        return;
      }
      const row = rows.length ? rows[0] : new Record(txApp.findCollectionByNameOrId("agent_directory"));
